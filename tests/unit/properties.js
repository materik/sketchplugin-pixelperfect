...
describe('properties', function() {
    it('invalid', function() {
        var component = src.Component.init(createLayer('Hej'));
        var properties = src.Properties.init(component);
        assert.equal(properties.count(), 0);
        var component = src.Component.init(createLayer('Hej:hej'));
        var properties = src.Properties.init(component);
        assert.equal(properties.count(), 0);
        var component = src.Component.init(createLayer('Hej [a:d]'));
        var properties = src.Properties.init(component);
        assert.equal(properties.count(), 0);
    });

    it('toString', function() {
        var properties = src.Properties.init(src.Component.init(createLayer('Hej')));
        assert.equal(properties.toString(), '<>')
        var properties = src.Properties.init(src.Component.init(createLayer('mr:h100')));
        ...
        var properties = src.Properties.init(src.Component.init(group));
    ...
    it('one valid', function() {
        ...
        var component = src.Component.init(createLayer('Hej:w100'));
        var properties = src.Properties.init(component);
    ...
    it('two valid', function() {
        var component = src.Component.init(createLayer('w100:h100'));
        var properties = src.Properties.init(component);
    ...
    it('find', function() {
        var properties = src.Properties.init(src.Component.init(createLayer('mr:h100:w20')));
    ...
    it('containsMargin', function() {
        var properties = src.Properties.init(src.Component.init(createLayer('h100:w20')));
        assert.equal(properties.containsMargin(), false);
        var properties = src.Properties.init(src.Component.init(createLayer('mr:h100:w20')));
        assert.equal(properties.containsMargin(), true);
        var properties = src.Properties.init(src.Component.init(createLayer('mb:h100:w20')));
        assert.equal(properties.containsMargin(), true);
    });

    it('containsMarginTopOrLeft', function() {
        var properties = src.Properties.init(src.Component.init(createLayer('b:r')));
        assert.equal(properties.containsMarginTopOrLeft(), false);
        var properties = src.Properties.init(src.Component.init(createLayer('t')));
        assert.equal(properties.containsMarginTopOrLeft(), true);
        var properties = src.Properties.init(src.Component.init(createLayer('t:b')));
        assert.equal(properties.containsMarginTopOrLeft(), false);
        var properties = src.Properties.init(src.Component.init(createLayer('l')));
        assert.equal(properties.containsMarginTopOrLeft(), true);
        var properties = src.Properties.init(src.Component.init(createLayer('l:r')));
        assert.equal(properties.containsMarginTopOrLeft(), false);
    });

    it('containsMarginRightOrBottom', function() {
        var properties = src.Properties.init(src.Component.init(createLayer('t:l')));
        assert.equal(properties.containsMarginRightOrBottom(), false);
        var properties = src.Properties.init(src.Component.init(createLayer('r')));
        assert.equal(properties.containsMarginRightOrBottom(), true);
        var properties = src.Properties.init(src.Component.init(createLayer('b')));
        assert.equal(properties.containsMarginRightOrBottom(), true);
    });

    it('containsMarginTopOrBottom', function() {
        var properties = src.Properties.init(src.Component.init(createLayer('r:l')));
    ...
    it('addZeroPadding', function() {
        ...
        var properties = src.Properties.init(src.Component.init(master));
        assert.equal(properties.count(), 0);
        properties.addZeroPadding();
        assert.equal(properties.count(), 4);
    });
    ...
    describe('apply', function() {
        it('default', function() {
            ...
            var properties = src.Properties.init(src.Component.init(layer));
            properties.apply()
    ...
    it('_raw', function() {
        var component = src.Component.init(createLayer('Component [1]'));
        var properties = src.Properties.init(component);
        assert.deepEqual(properties._raw(), ['pt1', 'pr1', 'pb1', 'pl1']);
        var component = src.Component.init(createLayer('Component [w100:h100]'));
        var properties = src.Properties.init(component);
        ...
        var component = src.Component.init(createLayer('h100%:h+90:h+90:w100%'));
        var properties = src.Properties.init(component);
        assert.deepEqual(properties._raw(), ['h100%', 'h+90', 'h+90', 'w100%']);
    });

    it('_sort', function() {
        var component = src.Component.init(createLayer('trbl'));
        var properties = Object.keys(src.Properties.init(component).keys())
        assert.deepEqual(properties, ['margin-right', 'margin-bottom', 'margin-top', 'margin-left']);
        var component = src.Component.init(createLayer('t:w100:b:h100'));
        var properties = Object.keys(src.Properties.init(component).keys())
        assert.deepEqual(properties, ['width', 'height', 'margin-bottom', 'margin-top']);
        var component = src.Component.init(createLayer('x10:c:w+10:w100%'));
        var properties = Object.keys(src.Properties.init(component).keys())
