...
describe('components-frame', function() {
    it('empty', function() {
        var frame = index.ComponentsFrame.init()
    ...
    it('toString', function() {
        var component1 = index.Component.init(createLayer('', 10, 40, 30, 40));
        var component2 = index.Component.init(createLayer('', 30, 20, 50, 60));
        var frame = index.ComponentsFrame.init(index.Components.items([component1, component2]))
    ...
    describe('getter', function() {
        ...
        it('x', function() {
            ...
            assert.equal(index.Components.init(NSArray.new([layer1, layer2])).frame().x(), 10);
            layer1.frame().setX(100);
            assert.equal(index.Components.init(NSArray.new([layer1, layer2])).frame().x(), 20);
        });

        it('y', function() {
            var layer1 = createLayer('1', 0, 10, 50, 60);
            var layer2 = createLayer('2', 0, 20, 100, 200);
            assert.equal(index.Components.init(NSArray.new([layer1, layer2])).frame().y(), 10);
            layer1.frame().setY(200);
            assert.equal(index.Components.init(NSArray.new([layer1, layer2])).frame().y(), 20);
        });

        it('width', function() {
            var layer1 = createLayer('1', 10, 0, 50, 60);
            var layer2 = createLayer('2', 20, 0, 100, 200);
            assert.equal(index.Components.init(NSArray.new([layer1, layer2])).frame().width(), 110);
            layer1.frame().setX(100);
            assert.equal(index.Components.init(NSArray.new([layer1, layer2])).frame().width(), 130);
        });

        it('height', function() {
            var layer1 = createLayer('1', 0, 10, 50, 60);
            var layer2 = createLayer('2', 0, 20, 100, 200);
            assert.equal(index.Components.init(NSArray.new([layer1, layer2])).frame().height(), 210);
            layer1.frame().setY(200);
            assert.equal(index.Components.init(NSArray.new([layer1, layer2])).frame().height(), 240);
        });

        it('maxWidth', function() {
            var layer1 = createLayer('1', 0, 0, 50, 60);
            var layer2 = createLayer('2', 0, 0, 100, 200);
            assert.equal(index.Components.init(NSArray.new([layer1, layer2])).frame().maxWidth(), 100);
            var layer2 = createLayer('w100%', 0, 0, 100, 200);
            assert.equal(index.Components.init(NSArray.new([layer1, layer2])).frame().maxWidth(), 50);
            var layer2 = createLayer('w100%%', 0, 0, 100, 200);
            assert.equal(index.Components.init(NSArray.new([layer1, layer2])).frame().maxWidth(), 100);
        });
        ...
        it('maxHeight', function() {
            var layer1 = createLayer('1', 0, 0, 50, 60);
            var layer2 = createLayer('2', 0, 0, 100, 200);
            assert.equal(index.Components.init(NSArray.new([layer1, layer2])).frame().maxHeight(), 200);
            var layer2 = createLayer('h100%', 0, 0, 100, 200);
            assert.equal(index.Components.init(NSArray.new([layer1, layer2])).frame().maxHeight(), 60);
            var layer2 = createLayer('h100%%', 0, 0, 100, 200);
            assert.equal(index.Components.init(NSArray.new([layer1, layer2])).frame().maxHeight(), 200);
        });
        ...
        it('right - ignore width percentage', function() {
            var component1 = index.Component.init(createLayer('', 10, 40, 30, 40));
            var component2 = index.Component.init(createLayer('w100%', 30, 20, 50, 60));
            var frame = index.ComponentsFrame.init(index.Components.items([component1, component2]))
            assert.equal(frame.right(), 40)
            var component1 = index.Component.init(createLayer('w100%', 10, 40, 30, 40));
            var component2 = index.Component.init(createLayer('', 30, 20, 50, 60));
            var frame = index.ComponentsFrame.init(index.Components.items([component1, component2]))
            assert.equal(frame.right(), 80)
            var component1 = index.Component.init(createLayer('w100%', 10, 40, 30, 40));
        ...
        it('right - ignore margin right', function() {
            var component1 = index.Component.init(createLayer('', 10, 40, 30, 40));
            var component2 = index.Component.init(createLayer('r', 30, 20, 50, 60));
            var frame = index.ComponentsFrame.init(index.Components.items([component1, component2]))
            assert.equal(frame.right(), 80)
            assert.equal(frame.right(true), 40)
            var component1 = index.Component.init(createLayer('r', 10, 40, 30, 40));
            var component2 = index.Component.init(createLayer('', 30, 20, 50, 60));
            var frame = index.ComponentsFrame.init(index.Components.items([component1, component2]))
            assert.equal(frame.right(), 80)
        ...
        it('bottom - ignore height percentage', function() {
            var component1 = index.Component.init(createLayer('', 10, 40, 30, 40));
            var component2 = index.Component.init(createLayer('h100%', 30, 20, 50, 60));
            var frame = index.ComponentsFrame.init(index.Components.items([component1, component2]))
            assert.equal(frame.bottom(), 80)
            var component1 = index.Component.init(createLayer('h100%', 10, 40, 30, 40));
            var component2 = index.Component.init(createLayer('', 30, 40, 50, 60));
            var frame = index.ComponentsFrame.init(index.Components.items([component1, component2]))
        ...
        it('bottom - ignore margin bottom', function() {
            var component1 = index.Component.init(createLayer('', 10, 40, 30, 40));
            var component2 = index.Component.init(createLayer('b', 30, 40, 50, 60));
            var frame = index.ComponentsFrame.init(index.Components.items([component1, component2]))
            assert.equal(frame.bottom(), 100)
            assert.equal(frame.bottom(true), 80)
            var component1 = index.Component.init(createLayer('b', 10, 40, 30, 40));
            var component2 = index.Component.init(createLayer('', 30, 40, 50, 60));
            var frame = index.ComponentsFrame.init(index.Components.items([component1, component2]))
            assert.equal(frame.bottom(), 100)
