...
describe('component', function() {
    ...
    it('localSymbol', function() {
        ...
        index.Component.apply(instance);
    ...
    it('textLayer', function() {
        ...
        index.Component.apply(textLayer);
    ...
    it('shape', function() {
        ...
        var component = index.Component.init(shape)
    ...
    it('isArtboard', function() {
        ...
        assert.equal(index.Component.init(layer).isArtboard(), false)
        var artboard = createArtboard()
        assert.equal(index.Component.init(artboard).isArtboard(), true)
        var group = createLayerGroup()
        assert.equal(index.Component.init(group).isArtboard(), false)
        var master = createSymbolMaster()
        assert.equal(index.Component.init(master).isArtboard(), false)
        var instance = createSymbolInstance(master)
        assert.equal(index.Component.init(instance).isArtboard(), false)
        var textLayer = createTextLayer();
        assert.equal(index.Component.init(textLayer).isArtboard(), false)
    })

    it('isGroup', function() {
        var layer = createLayer()
        assert.equal(index.Component.init(layer).isGroup(), false)
        var artboard = createArtboard()
        assert.equal(index.Component.init(artboard).isGroup(), false)
        var group = createLayerGroup()
        assert.equal(index.Component.init(group).isGroup(), true)
        var master = createSymbolMaster()
        assert.equal(index.Component.init(master).isGroup(), false)
        var instance = createSymbolInstance(master)
        assert.equal(index.Component.init(instance).isGroup(), false)
        var textLayer = createTextLayer();
        assert.equal(index.Component.init(textLayer).isGroup(), false)
    })

    it('isSymbolMaster', function() {
        var layer = createLayer()
        assert.equal(index.Component.init(layer).isSymbolMaster(), false)
        var artboard = createArtboard()
        assert.equal(index.Component.init(artboard).isSymbolMaster(), false)
        var group = createLayerGroup()
        assert.equal(index.Component.init(group).isSymbolMaster(), false)
        var master = createSymbolMaster()
        assert.equal(index.Component.init(master).isSymbolMaster(), true)
        var instance = createSymbolInstance(master)
        assert.equal(index.Component.init(instance).isSymbolMaster(), false)
        var textLayer = createTextLayer();
        assert.equal(index.Component.init(textLayer).isSymbolMaster(), false)
    })

    it('isArtboardOrSymbolMaster', function() {
        var layer = createLayer()
        assert.equal(index.Component.init(layer).isArtboardOrSymbolMaster(), false)
        var artboard = createArtboard()
        assert.equal(index.Component.init(artboard).isArtboardOrSymbolMaster(), true)
        var group = createLayerGroup()
        assert.equal(index.Component.init(group).isArtboardOrSymbolMaster(), false)
        var master = createSymbolMaster()
        assert.equal(index.Component.init(master).isArtboardOrSymbolMaster(), true)
        var instance = createSymbolInstance(master)
        assert.equal(index.Component.init(instance).isArtboardOrSymbolMaster(), false)
        var textLayer = createTextLayer();
        assert.equal(index.Component.init(textLayer).isArtboardOrSymbolMaster(), false)
    })

    it('shouldApply', function() {
        var layer = createLayer('');
        assert.equal(index.Component.init(layer).shouldApply(), true);
        layer.setIsVisible(false);
        assert.equal(index.Component.init(layer).shouldApply(), false);
    ...
    it('hasParent', function() {
        var layer = createLayer();
        assert.equal(index.Component.init(layer).hasParent(), false)
        var group = createLayerGroup()
        group.insertLayer_afterLayerOrAtEnd(layer)
        assert.equal(index.Component.init(layer).hasParent(), true)
    })

    it('roundToPixel', function() {
        var component = index.Component.init(createLayer('', 1.1, 2.2, 3.3, 4.5));
    ...
    describe('frame', function() {
        it('leftInParent', function() {
            ...
            assert.equal(index.Component.init(layer1).leftInParent(), 0);
            var group = createLayerGroup();
            group.insertLayer_afterLayerOrAtEnd(layer1);
            assert.equal(index.Component.init(layer1).leftInParent(), 5);
            assert.equal(index.Component.init(layer1).leftInParent(true), 0);
            group.insertLayer_afterLayerOrAtEnd(layer2);
            assert.equal(index.Component.init(layer1).leftInParent(), 5);
            assert.equal(index.Component.init(layer1).leftInParent(true), 10);
            assert.equal(index.Component.init(layer2).leftInParent(), 5);
            assert.equal(index.Component.init(layer2).leftInParent(true), 5);
        ...
        it('topInParent', function() {
            var layer1 = createLayer('1', 0, 5, 50, 60);
            var layer2 = createLayer('2', 0, 10, 100, 200);
            assert.equal(index.Component.init(layer1).topInParent(), 0);
            var group = createLayerGroup();
            group.insertLayer_afterLayerOrAtEnd(layer1);
            group.insertLayer_afterLayerOrAtEnd(layer1);
            assert.equal(index.Component.init(layer1).topInParent(), 5);
            assert.equal(index.Component.init(layer1).topInParent(true), 0);
            group.insertLayer_afterLayerOrAtEnd(layer2);
            assert.equal(index.Component.init(layer1).topInParent(), 5);
            assert.equal(index.Component.init(layer1).topInParent(true), 10);
            assert.equal(index.Component.init(layer2).topInParent(), 5);
            assert.equal(index.Component.init(layer2).topInParent(true), 5);
        ...
        it('widthOfParent', function() {
            ...
            assert.equal(index.Component.init(layer1).widthOfParent(), 0);
            var group = createLayerGroup();
            group.insertLayer_afterLayerOrAtEnd(layer1);
            group.insertLayer_afterLayerOrAtEnd(layer2);
            assert.equal(index.Component.init(layer1).widthOfParent(), 100);
            assert.equal(index.Component.init(layer1).widthOfParent(false, true), 100);
            assert.equal(index.Component.init(layer2).widthOfParent(), 100);
            assert.equal(index.Component.init(layer2).widthOfParent(false, true), 50);
            var artboard = createArtboard('Arboard', 0, 0, 150, 300);
            artboard.insertLayer_afterLayerOrAtEnd(layer1);
            artboard.insertLayer_afterLayerOrAtEnd(layer2);
            assert.equal(index.Component.init(layer1).widthOfParent(), 150);
            ...
            assert.equal(index.Component.init(layer1).widthOfParent(), 3);
            var artboard = createArtboard('artboard', 5, 6, 7, 8);
            artboard.insertLayer_afterLayerOrAtEnd(layerGroup);
            assert.equal(index.Component.init(layer1).widthOfParent(), 3);
            layerGroup.setName('w100%');
            assert.equal(index.Component.init(layer1).widthOfParent(), 7);

            var layer = createLayer('', 0, 0, 10, 20);
            var layerGroup = createLayerGroup('', 0, 0, 100, 200);
            layerGroup.insertLayer_afterLayerOrAtEnd(layer);
            assert.equal(index.Component.init(layer).widthOfParent(true), 100);
        });

        it('heightOfParent', function() {
            var layer1 = createLayer('1', 0, 0, 50, 60);
            var layer2 = createLayer('2', 0, 0, 100, 200);
            assert.equal(index.Component.init(layer1).heightOfParent(), 0);
            var group = createLayerGroup();
            group.insertLayer_afterLayerOrAtEnd(layer1);
            group.insertLayer_afterLayerOrAtEnd(layer2);
            assert.equal(index.Component.init(layer1).heightOfParent(), 200);
            assert.equal(index.Component.init(layer1).heightOfParent(false, true), 200);
            assert.equal(index.Component.init(layer2).heightOfParent(), 200);
            assert.equal(index.Component.init(layer2).heightOfParent(false, true), 60);
            var artboard = createArtboard('Arboard', 0, 0, 150, 300);
            artboard.insertLayer_afterLayerOrAtEnd(layer1);
            artboard.insertLayer_afterLayerOrAtEnd(layer2);
            assert.equal(index.Component.init(layer1).heightOfParent(), 300);
            ...
            layerGroup.insertLayer_afterLayerOrAtEnd(layer1);
            layerGroup.insertLayer_afterLayerOrAtEnd(layer2);
            assert.equal(index.Component.init(layer1).heightOfParent(), 3);
            var artboard = createArtboard('artboard', 5, 6, 7, 8);
            artboard.insertLayer_afterLayerOrAtEnd(layerGroup);
            assert.equal(index.Component.init(layer1).heightOfParent(), 3);
            layerGroup.setName('h100%');
            assert.equal(index.Component.init(layer1).heightOfParent(), 8);

            var layer = createLayer('', 0, 0, 10, 20);
            var layerGroup = createLayerGroup('', 0, 0, 100, 200);
            layerGroup.insertLayer_afterLayerOrAtEnd(layer);
            assert.equal(index.Component.init(layer).heightOfParent(true), 200);
        });
